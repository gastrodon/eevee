...
pub fn serialize<S: Serializer>(matrix: &Matrix<f64>, serializer: S) -> Result<S::Ok, S::Error> {
...
pub fn deserialize_flat<'de, D: Deserializer<'de>>(
    deserializer: D,
...
pub fn deserialize_square<'de, D: Deserializer<'de>>(
    deserializer: D,
) -> Result<Matrix<f64>, D::Error> {
    Vec::<u64>::deserialize(deserializer).map(|v| {
        // Convert u64 bits back to f64 values
